...
2. 如果docker registry需要进行授权时，registry将会返回401 Unauthorized响应，同时在返回的头信息中包含了docker client如何进行认证的信息
...
```
Www-Authenticate: Bearer realm="http://172.16.137.217:8080/auth",service="Docker registry",scope="repository:samalba/my-app:pull"
```
...
public class RegistryAuthServer {
   ...
   public RegistryTokenResponse auth(HttpServletRequest request) {
    ...
    //getPublicKeyId()请使用下面提供的kid-tools进行生成
...
```
针对kid的生成可以参考[https://github.com/mojo-zd/kid-tools](https://github.com/mojo-zd/kid-tools).
这是一个使用go实现的生成kid工具.目前生成了mac、linux的可执行二进制文件.如果要使用windows版本或者其他版本的可行执行文件可以自己下载源码进行交叉编译.
该工具使用很简单:
1. 从上述地址下载可执行文件
2. ./kid-toos  certfilepath/certfile.crt keyfilepath/privatefile.key
